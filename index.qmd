...
train_df %>%
  ...
  slice_max(n, n = 10) %>%
  ggplot(aes(x = n, y = reorder(eventName, n))) +
  geom_col(fill = "steelblue") +
  labs(title = "Top 10 Event Names in Training Data", x = "Frequency", y = "Event Name") +
  theme_minimal()
```
...
train_df %>%
  count(processName, sort = TRUE) %>%
  slice_max(n, n = 10) %>%
  ggplot(aes(x = n, y = reorder(processName, n))) +
  geom_col(fill = "darkorchid") +
  labs(title = "Top 10 Process Names in Training Data", x = "Frequency", y = "Process Name") +
  theme_minimal()
...
train_df %>%
  ...
  labs(title = "System Events Distribution by Hour (Training Data)",
...
```{r}
test_df %>%
  mutate(evil = factor(evil, levels = c(0, 1), labels = c("Benign (0)", "Malicious (1)"))) %>%
  ggplot(aes(x = evil, fill = evil)) +
  geom_bar() +
  labs(title = "Malicious vs. Benign Events (Test Set)", x = "Evil Label", y = "Count") +
